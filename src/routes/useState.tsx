...
import { ReactNode, useState, Component, Children, useEffect } from 'react'
  ...
  return (
    <>
      <div className='grid grid-cols-4 grid-rows-6 items-center justify-center h-[calc(100dvh-64px)] w-[calc(97px*4+12px*3)] text-3xl gap-3 py-4 m-auto font-bold max-sm:w-full max-sm:p-4 mt-16'>
        ...
        <button className='rounded-3xl h-full bg-stone-500/10 hover: cursor-pointer text-emerald-700 hover:text-emerald-600 active:bg-emerald-700 active:text-white col-start-1 col-end-3 row-start-6' onClick={()=>{setCalcScreen(addCharOnCalc(CalcScreen,'0'))}}>0</button>
        <button className='rounded-3xl h-full bg-stone-500/10 hover: cursor-pointer text-cyan-600 hover:text-cyan-500 active:bg-cyan-600 active:text-white col-start-2 row-start-2' onClick={()=>{setCalcScreen(addCharOnCalc(CalcScreen,'()'))}}>( )</button>
...
function addCharOnCalc(count:string, numberToAdd:string):string{
  ...
  const lestChar = count[count.length - 1]
  ...
  if(numberToAdd == ','){
    ...
    if(isNaN(parseInt(lestChar)) && lestChar != ','){
      return count+'0,'
    }
    
    if(isNaN(Number(lestChar))){
      return count
    }
  ...
  // if(numberToAdd == '+/-'){
  //   console.group('mais ou menos')
  //   console.log(lestChar);
  //   console.groupEnd()
    
  //   if(isNaN(Number(lestChar)) && lestChar != ','){
  ...
  if(numberToAdd == '()'){
    ...
    if (count.split('(').length == count.split(')').length) {
      if(lestChar == ')' || lestChar == '%' || lestChar == ',' || !isNaN(Number(lestChar))){
        return count+'*('
      }
      return count+'('
    }
  ...
  if(numberToAdd != '%' && lestChar != '(' && lestChar != ')' && isNaN(Number(numberToAdd)) && isNaN(Number(lestChar))){
...
function calcular(conta:string): string {
  ...
  try{
    ...
    return String(
      eval(
        conta.split('(').length == conta.split(')').length?

        conta.replaceAll('÷','/').replaceAll(',','.').replaceAll('%','/100'):

        (
          conta+')'.repeat(
            (conta.split('(').length - conta.split(')').length)
          )
        ).replaceAll('÷','/').replaceAll(',','.').replaceAll('%','/100')
